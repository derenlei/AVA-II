...
def main(unused_argv):
  ...
  '''
  train_data=np.load('train_image_matrix_small.npy').astype(np.float32)
  #print('train_data',train_data.shape)
  train_labels=np.load('train_labels_small.npy').astype(np.int64)
  train_labels = train_labels-1
  #train_labels = tf.one_hot(train_labels,depth=14)
  #print('train_labels',train_labels.shape)
  eval_data=np.load('val_image_matrix_small.npy').astype(np.float32)
  #print('eval_data',eval_data.shape)
  eval_labels=np.load('val_labels_small.npy').astype(np.int64)
  eval_labels = eval_labels-1
  #eval_labels = tf.one_hot(eval_labels,depth=14)
  #print('eval_label',eval_labels.shape)
  # Create the Estimator
  mnist_classifier = tf.estimator.Estimator(
      model_fn=resnet50, model_dir="test")
