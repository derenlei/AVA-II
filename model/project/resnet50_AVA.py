...
def resnet50(features, labels, mode):
  ...
  input_layer = tf.reshape(features["x"], [-1, 224, 224, 3])
    ...
    #input_layer = tf.map_fn(lambda img: tf.image.random_brightness(img,max_delta=63), input_layer)
    #input_layer = tf.map_fn(lambda img: tf.image.random_contrast(img,lower=0.2,upper=1.8), input_layer)
  ...
  #input_layer = tf.constant(features["x"])
...
def trainInput_fn(features, labels, batch_size):
  """An input function for training"""
  feature_init = tf.placeholder(tf.float32,shape=(features.shape[0],features.shape[1]))
  feature = tf.Variable(feature_init)
  sess.run(tf.initialize_all_vairables(),feed_dict={feature_init:features})

  # Convert the inputs to a Dataset.
  features_dict = {'x':features}
...
def evalInput_fn(features, labels, batch_size=128):
...
def main(unused_argv):
  ...
  '''
  Usually I store the array used numpy.save, this is use to get the data when the storage is full,
  (train_data,val_data) = input_ava.input_data('AVA-II','/data/ece194n/AVA/keyframes','images')
  (training_image_matrix,train_labels,val_image_matrix,val_labels)=input_ava.read_image(train_data,val_data,'AVA-II','/data/ece194n/AVA/keyframes','images',False)
  train_data = training_image_matrix.astype(np.float32)
  del training_image_matrix
  train_labels=train_labels.astype(np.int64)
  del train_labels
  train_labels = train_labels-1
  eval_data = val_image_matrix.astype(np.float32)
  del val_image_matrix
  eval_labels=val_labels.astype(np.int64)
  del val_labels
  eval_labels = eval_labels-1
  '''
  train_data=np.load('train_image_matrix.npy').astype(np.float32)
  train_labels=np.load('train_labels.npy').astype(np.int64)
  train_labels = train_labels-1
  eval_data=np.load('val_image_matrix.npy').astype(np.float32)
  eval_labels=np.load('val_labels.npy').astype(np.int64)
  eval_labels = eval_labels-1
  # Create the Estimator
  #shuffle data
  randomized = np.arange(train_data.shape[0])
  np.random.shuffle(randomized)
  train_data = train_data[randomized,:,:,:]
  train_labels = train_labels[randomized]
  del randomized
  session_config = tf.ConfigProto()
  session_config.gpu_options.allow_growth = True
  #session_config.gpu_options.per_process_gpu_memory_fraction = 0.5
  config = tf.estimator.RunConfig(session_config = session_config)
  ava_classifier = tf.estimator.Estimator(
      model_fn=resnet50, model_dir="model_resnet50_0.0001", config = config)
  ...
  tf.estimator.train_and_evaluate(ava_classifier, train_spec, eval_spec)
